...
<div class="card mb-4">
    <div class="card-body">
        <form method="POST" action="{{ route('admin.forms.update', $form) }}" id="formBuilder">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3 mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="schedule_type" id="scheduleType"
                        ...
                        <option value="daily"
                            {{ old('schedule_type', $form->schedule_type) == 'daily' ? 'selected' : '' }}>Daily</option>
                        <option value="weekly"
                            {{ old('schedule_type', $form->schedule_type) == 'weekly' ? 'selected' : '' }}>Weekly
                        </option>
                        <option value="custom"
                            {{ old('schedule_type', $form->schedule_type) == 'custom' ? 'selected' : '' }}>Custom
                        </option>
                    </select>
            ...
            <div class="mb-3" id="scheduleDaysContainer"
                style="display: {{ old('schedule_type', $form->schedule_type) == 'weekly' ? 'block' : 'none' }};">
                <label class="form-label fw-semibold">Hari</label>
                <div class="d-flex flex-wrap gap-3">
                    @php
                    $days =
                    ['Mon'=>'Senin','Tue'=>'Selasa','Wed'=>'Rabu','Thu'=>'Kamis','Fri'=>'Jumat','Sat'=>'Sabtu','Sun'=>'Minggu'];
                    $savedDays = old('schedule_days', is_array($form->schedule_days) ? $form->schedule_days :
                    json_decode($form->schedule_days, true) ?? []);
                    @endphp
                    @foreach($days as $val => $label)
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="schedule_days[]" value="{{ $val }}"
                            id="day_{{ $val }}" {{ in_array($val, $savedDays) ? 'checked' : '' }}>
            ...
            <div class="row g-3 mb-3" id="customIntervalContainer"
                style="display: {{ old('schedule_type', $form->schedule_type) == 'custom' ? 'flex' : 'none' }};">
                <div class="col-md-6">
                    <label class="form-label fw-semibold">Interval (hari)</label>
                    <input type="number" name="schedule_interval" class="form-control"
                        value="{{ old('schedule_interval', $form->schedule_interval) }}" min="1">
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="description" class="form-control"
                    rows="2">{{ old('description', $form->description) }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="assigned_users[]" class="form-select" multiple size="4">
                    @foreach($users as $u)
                    <option value="{{ $u->id }}"
                        {{ in_array($u->id, old('assigned_users', $assignedUserIds ?? [])) ? 'selected' : '' }}>
                        {{ $u->name }} ({{ $u->email }})
                    </option>
                    @endforeach
                </select>
            ...
            <div id="itemsContainer">
                <!-- Existing items loaded by JS below -->
            </div>
...
function addItem(data = null) {
    ...
    let typeOptions = itemTypes.map(t =>
        `<option value="${t.value}" ${data?.field_type == t.value ? 'selected' : ''}>${t.label}</option>`).join('');
            ...
            <div class="card-body">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="items[${idx}][label]" class="form-control" placeholder="Contoh: IP Address" value="${data?.label || ''}" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label small fw-semibold">Tipe Field</label>
                        <select name="items[${idx}][field_type]" class="form-select item-type" onchange="toggleOptions(this, ${idx})" required>
                            ${typeOptions}
                        </select>
                    </div>
                    <div class="col-md-3 d-flex align-items-end pb-1">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="items[${idx}][is_required]" id="req_${idx}" ${data?.is_required ? 'checked' : ''}>
                            <label class="form-check-label" for="req_${idx}">Wajib diisi</label>
                        </div>
                    </div>
                    <div class="col-md-12 options-row-${idx}" style="display: ${['checkbox','radio','dropdown','signal'].includes(data?.field_type) ? 'block' : 'none'};">
                        <label class="form-label small fw-semibold">Opsi (pisahkan dengan koma)</label>
                        <input type="text" name="items[${idx}][options_raw]" class="form-control" placeholder="Online, Offline, Maintenance" value="${data?.options_raw || ''}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold">Placeholder</label>
                        <input type="text" name="items[${idx}][placeholder]" class="form-control" value="${data?.placeholder || ''}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold">Helper Text</label>
                        <input type="text" name="items[${idx}][helper_text]" class="form-control" value="${data?.helper_text || ''}">
                    </div>
                </div>
            </div>
        </div>
...
foreach($form->items as $item) {
    $existingItems[] = [
        ...
        'options_raw' => is_array($item->options) ? implode(', ', $item->options) : '',
...
@php
if (old('items')) {
    echo 'const oldItems = '.json_encode(old('items')).
    ';';
    echo 'oldItems.forEach(item => addItem(item));';
}
@endphp
</script>
@endsection