...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .danger-text { color: #b42318; font-weight: bold; }.answer { margin-bottom: 3px; padding-bottom: 3px; border-bottom: 1px solid #e7edf4; }.answer:last-child { margin-bottom: 0; padding-bottom: 0; border-bottom: 0; }.checkbox-option { margin: 2px 0 0 8px; }.checkbox-mark { display: inline-block; width: 14px; text-align: center; }
...
<body>
    ...
    <section class="section"><h2 class="section-title">Detail Daily Activity</h2><table class="data"><thead><tr><th>Tanggal</th><th>Staff</th><th>Jenis</th><th>Kategori</th><th>Aktivitas</th><th>Status</th><th>Catatan</th><th>Penugasan</th><th>Diperbarui</th></tr></thead><tbody>
        @forelse($dailyActivities as $activity) @php($status = ['completed' => ['Selesai', 'status-completed'], 'in_progress' => ['Dalam Proses', 'status-progress'], 'blocked' => ['Terhambat', 'status-blocked']][$activity->status] ?? [$activity->status, ''])<tr><td class="nowrap">{{ $activity->activity_date?->format('d/m/Y') }}</td><td>{{ $activity->user?->name ?? '-' }}</td><td>{{ $activity->type === 'ticketing' ? 'Ticketing' : 'Daily Activity' }}</td><td>{{ $activity->category }}</td><td>{{ $activity->activity }}{{ $activity->ticket_url ? ' - '.$activity->ticket_url : '' }}</td><td><span class="status {{ $status[1] }}">{{ $status[0] }}</span></td><td>{{ $activity->notes ?: '-' }}</td><td>{{ $activity->assigner?->name ?? 'Mandiri' }}</td><td class="nowrap">{{ $activity->updated_at?->format('d/m/Y') }}<br><span class="muted">{{ $activity->updated_at?->format('H:i') }}</span></td></tr>@empty<tr><td class="empty" colspan="9">Belum ada daily activity pada filter yang dipilih.</td></tr>@endforelse
    </tbody></table></section>
    <section class="section detail-section"><h2 class="section-title">Detail Submission Checklist</h2><p class="section-note">Jawaban yang memerlukan perhatian ditampilkan dalam warna merah.</p><table class="data"><thead><tr><th style="width:17%">Form</th><th style="width:12%">User</th><th style="width:9%">Tanggal</th><th style="width:7%">Waktu</th><th style="width:10%">Status</th><th>Jawaban Checklist</th></tr></thead><tbody>
        @forelse($submissions as $submission) @php($flagged = $submission->answers->where('is_flagged', true)->count())<tr><td>{{ $submission->form?->title ?? '-' }}</td><td>{{ $submission->submitter?->name ?? '-' }}</td><td class="nowrap">{{ $submission->submission_date?->format('d/m/Y') }}</td><td class="nowrap">{{ $submission->submitted_at?->format('H:i') ?? '-' }}</td><td><span class="status {{ $flagged ? 'status-issue' : 'status-ok' }}">{{ $flagged ? $flagged . ' masalah' : 'OK' }}</span></td><td>@forelse($submission->answers as $answer)<div class="answer {{ $answer->is_flagged ? 'danger-text' : '' }}"><strong>{{ $answer->formItem?->label ?? 'Item' }}</strong>@if($answer->formItem?->field_type === 'checkbox' && filled($answer->formItem?->options)) @php($checkboxStatuses = $answer->checkboxStatuses()) @foreach($answer->formItem->options as $option)<div class="checkbox-option">{{ $option }} — Normal {{ ($checkboxStatuses[$option] ?? null) === 'normal' ? '☑' : '☐' }} | Tidak Normal {{ ($checkboxStatuses[$option] ?? null) === 'tidak_normal' ? '☑' : '☐' }}</div>@endforeach @else: {{ $answer->formItem?->field_type === 'photo' ? count($answer->photoPaths()).' foto' : ($answer->answer_value ?: '-') }}@endif</div>@empty<span class="muted">Tidak ada jawaban.</span>@endforelse</td></tr>@empty<tr><td class="empty" colspan="6">Belum ada submission pada filter yang dipilih.</td></tr>@endforelse
    </tbody></table></section>
</body>
</html>