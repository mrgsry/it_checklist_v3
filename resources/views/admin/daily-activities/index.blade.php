    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead><tr><th>Tanggal</th><th>Staff</th><th>Aktivitas</th><th>Status</th><th>Catatan</th><th>Diperbarui</th><th class="text-end">Aksi</th></tr></thead>
                <tbody>
                    @forelse($activities as $activity)
                        ...
                        <tr><td>{{ $activity->activity_date->isoFormat('D MMM Y') }}</td><td class="fw-semibold">{{ $activity->user->name }}</td><td>{{ $activity->activity }}</td><td><span class="badge text-bg-{{ $status[0] }}">{{ $status[1] }}</span></td><td class="text-muted">{{ $activity->notes ?: '-' }}</td><td>{{ $activity->updated_at->format('d M Y, H:i') }}</td><td class="text-end"><a href="https://wa.me/?text={{ rawurlencode($whatsAppMessage) }}" class="btn btn-sm btn-success" target="_blank" rel="noopener noreferrer" title="Kirim detail tugas ke WhatsApp" aria-label="Kirim detail tugas ke WhatsApp"><i class="fab fa-whatsapp"></i></a></td></tr>
                    @empty
                        <tr><td colspan="7" class="text-center text-muted py-5"><i class="fas fa-clipboard-list d-block fa-2x mb-2"></i>Belum ada daily activity yang sesuai.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
