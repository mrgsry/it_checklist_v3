...
@section('content')
@php
    $canWriteChecklist = auth()->user()->hasModuleAccess('checklist', 'write');
...
<div class="row g-3 mb-4">
    <div class="col-12 col-lg-7"><div class="card-createspace h-100"><div class="card-body"><div class="d-flex justify-content-between align-items-center mb-3"><h5 class="font-headline font-semibold mb-0"><i class="fas fa-tasks me-2"></i>Checklist hari ini</h5><a href="{{ route('user.checklist.index') }}" class="btn-createspace btn-sm btn-ghost">Lihat semua</a></div>@forelse($formsDue as $form)@php($submission = $submittedToday->firstWhere('form_id', $form->id))<div class="d-flex align-items-center gap-3 py-2 border-bottom"><i class="fas {{ $submission ? 'fa-check-circle text-success' : 'fa-clock text-warning' }} fa-lg"></i><div class="flex-grow-1"><div class="fw-semibold">{{ $form->title }}</div><small class="text-muted">{{ $submission ? 'Sudah diisi hari ini' : 'Menunggu submit' }}</small></div>@if(!$submission && $canWriteChecklist)<a href="{{ route('user.checklist.fill', $form->id) }}" class="btn-createspace btn-sm btn-primary">Isi</a>@elseif(!$submission)<span class="small text-muted"><i class="fas fa-lock me-1"></i>Read only</span>@endif</div>@empty<div class="text-center text-muted py-4">Tidak ada form terjadwal hari ini.</div>@endforelse</div></div></div>
